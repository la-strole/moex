...
function ajax_get_primary_board(){
  ...
  // 3. Make Sell button disable until bid exist 
  if (!document.getElementById("button_sell").classList.contains('disabled')){
    document.getElementById("button_sell").classList.add("disabled");
  }

  // 4. Get value from ticker select field
  let text = document.getElementById("sell_select").value;
...
function change_detail_info(symbol, market){
  ...
  if (result){
    ...
    if (result["BID"][1]){
      document.getElementById('bid').innerHTML = result['BID'][1] + '\u00A0' + symbol_val;
    }
    else{
      document.getElementById('bid').innerHTML = "Нет спроса";
    }
    document.getElementById('depo_number').innerHTML = tickers_max_dict[symbol] + '\u00A0' + 'ед';

    /* If BID exist make buy button enable */

    if (result['BID'][1] != null){
      if (document.getElementById("button_sell").classList.contains('disabled')){
        document.getElementById("button_sell").classList.remove('disabled');
      }
    }
...
if (document.readyState === 'loading') {  // document still loaded
  ...
  document.addEventListener('DOMContentLoaded', function(){
        document.getElementById("sell_select").addEventListener('change', ajax_get_primary_board);
                                                          }
                            );
  document.addEventListener('DOMContentLoaded', function(){
        if (ticker_exist){
            ajax_get_primary_board(document.getElementById("sell_select").value.split(' ')[0]);
        }  
        })
}
