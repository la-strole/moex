...
def init(database):
    ...
    :param database: database name from app.config (look at __init__.py)
    :return: None
    """

    scheduler_function(database)
...
def first_mail(database):
    ...
    for row in mail_list:
        ...
        try:
            ...
            helpers_functions.app_log_add(f"Error. schedule.py first_mail(): Can not sen email to {row.get('email')}")
            continue
...
def second_mail(database):
    ...
    helpers_functions.app_log_add(f"Success. schedule.py second_mail(): Send {sent_mail_number} with notifications.")
    return sent_mail_number
...
def scheduler_function(database):
    """
    Get prices for tickers from moex every interval
    :param database: database name from app.config (look at __init__.py)
    :return: None
    """
    ...
    # Add job to check current price for notify users every hours = 1
    scheduler.add_job(func=lambda: first_mail(database), trigger="interval", hours=1,
                      timezone=pytz.timezone('Europe/Moscow'))

    # Add job for daily update tickers listing days=1
    scheduler.add_job(func=lambda: update_symbols(database), trigger="interval", days=1,
                      timezone=pytz.timezone('Europe/Moscow'), start_date='2021-12-20 10:30:00')

    # Add job for daily check current price for notify users every day as second emails days=1
    scheduler.add_job(func=lambda: second_mail(database), trigger="interval", days=3,
                      timezone=pytz.timezone('Europe/Moscow'), start_date='2021-12-20 10:40:00')
